...
app.post('/api/f1-commentary', async (req, res) => {
  ...
  if (!ai) {
    const fallbacks: Record<string, Record<string, string>> = {
      ...
      technical: {
        fr: `📊 Analyse télémétrique (YOLOv8 + LightGBM): ${driver || 'Le pilote'} applique 98% de pression de freinage à 112m. Dégradation pneumatiqueestimée à 42%. Différentiel de vitesse en sortie: +14.2 km/h.`,
  ...
  try {
    ...
    const systemInstructions: Record<string, string> = {
      ...
      technical:
        'Tu es un ingénieur de piste F1 et Data Scientist. Donne une explication technique concise basée sur la télémétrie, la vitesse, la pression desfreins et la dégradation pneumatique.',
      stats:
